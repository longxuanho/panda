...
import modalTemplate from './tbisDetailsViewHistoryViewKhacTabModal.html';
...
class TbisDetailsViewHistoryViewKhacTab {
        ...
        this.helpers({
            tbisHistories() {
                return tbisHistoriesDataService.query(
                    {'phan_loai.nhom': { $ne: 'Sửa chữa nhỏ'}
                });
            }
        });
    ...
    open(event, tbisHistoryId) {
        ...
        this.$mdDialog.show({
                ...
                this.updateSelectedTbisHistory = () => {
                    try {
                        if (this.selectedTbisHistory.ghi_chu.html)
                            this.selectedTbisHistory.ghi_chu.text = $('#tbis-details-view-tbis-history-khac-tab-modal iframe').contents().find("body").text() || this.selectedTbisHistory.ghi_chu.html;
            ...
            controllerAs: 'tbisDetailsViewHistoryViewKhacTabModal',
